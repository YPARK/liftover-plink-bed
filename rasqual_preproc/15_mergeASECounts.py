...
parser.add_argument("--sample_list", help = "Tab separated text file mapping sample names to file paths (columns: genotype_id, sample_path).")
args = parser.parse_args()

#Load sample names from disk
sample_list = list()                    # tuples of (genotype_id, sample_id)
sample_names = list()                   # genotype_id (SIC!) recheck if wrong in doc
...
for sample_number in range(len(sample_list)):
	file_name = sample_list[sample_number][1]
